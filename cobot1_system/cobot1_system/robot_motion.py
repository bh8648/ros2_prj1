...
# 카메라 픽셀 좌표 -> 로봇 베이스 평면 좌표(mm) 호모그래피.
# tools/calibrate_corners.py로 측정한 바구니 4코너 픽셀 ↔ 위 로봇 코너로 산출.
#   TL px(504,442)  TR px(85,455)  BL px(476,43)  BR px(95,34)
# 카메라 위치/각도가 바뀌면 재측정 후 이 행렬을 갱신해야 한다.
IMG2ROBOT_H = [
    [-0.9520389884, 0.1656254358, 678.9132940081],
    [0.0033277702, 0.8141876555, -89.2560534116],
    [-0.0003995396, 0.0003028848, 1.0000000000],
]
...
def press_move():
    ...
    dsr.movel([prime_pose[0], prime_pose[1], 0, prime_pose[3], prime_pose[4], prime_pose[5]], VELOCITY, ACC, ref = dsr.DR_BASE)
...
def grapping_side_object_left(angle):  # 물건의 x좌표가 370 이하일때 쓰는 grapping 함수
        ...
        dsr.movel([0, 0, 0, 0, angle, 0], VELOCITY, ACC, ref=dsr.DR_TOOL)
        dsr.wait(0.5)
...
def pick_object(center_x: float, center_y: float, angle: float) -> bool:
    ...
    near_edge = (
        robot_x < top_left[0] + 75.00
        or robot_y < bot_left[1] + 75.00
        or robot_x > top_right[0] - 75.00
        or robot_y > top_right[1] - 75.00
    )
