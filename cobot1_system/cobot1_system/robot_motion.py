...
def pick_up(max_try=5):  # z축 높이 차이에 따라 그리퍼 위치 조절 후 잡기
    ...
    for attempt in range(1, max_try + 1):
        ...
        if grip_checking():  # 들어올린 상태에서 힘 체크
            _node.get_logger().info("잡기 성공")
            break
        else:
            _node.get_logger().info(f"잡기 실")
    else:
...
def grapping_side_object_right(angle):  # 물건의 x좌표가 370 이하일때 쓰는 grapping 함수
    ...
    pose2[2] /= 1.8
    dsr.movel(pose2, VELOCITY, ACC, ref=dsr.DR_BASE)
    pose6 = dsr.get_current_posx()[0]
    pose6[0] += 25.00
    dsr.movel(pose6, VELOCITY, ACC, ref = dsr.DR_BASE)
...
def pick_object(center_x: float, center_y: float, angle: float) -> bool:
    ...
    # print는 ros2 launch에서 버퍼링돼 안 보이므로 ROS 로거로 출력(launch 화면에 확실히 뜸)
    ...
    if near_edge:
        ...
        z_top = dsr.get_current_posx()[0][2]
        _node.get_logger().info(
            f"[edge] 측정 z_top={z_top:.1f} → "
