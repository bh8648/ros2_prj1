...
class CentralNode(Node):
    ...
    def __init__(self):
        ...
        self.update_inventory_client = self.create_client(
        ...
        # /conveyor/run 토픽으로 컨베이어 벨트에 구동 신호를 보낸다
        # 추가 부분
        self.conveyor_pub = self.create_publisher(Bool, "/conveyor/run", 10)
        # PLACED 이후 3초 딜레이 타이머 핸들 (None = 예약 없음)
        self._conveyor_timer = None
    ...
    def _feedback_callback(self, feedback):
        ...
        # PLACED feedback을 받은 순간에만 DB 차감 요청 + 컨베이어 예약
    ...
    def _request_inventory_update(self, label_id, item_name, moved_count):
        ...
        if not self.update_inventory_client.service_is_ready():
            self.get_logger().warn(
                "/update_inventory 서비스 준비 안 됨. db_node 확인 필요"
            )
            return
    ...
    def _schedule_conveyor(self):  # 추가
        # PLACED가 중복으로 들어와도 타이머는 한 번만 잡는다
        if self._conveyor_timer is not None:
            return
        self._conveyor_timer = self.create_timer(3.0, self._fire_conveyor)

    def _fire_conveyor(self):  # 추가
        # 타이머 1회 실행 후 즉시 제거 (반복 타이머 방지)
        self.destroy_timer(self._conveyor_timer)
        self._conveyor_timer = None
        msg = Bool()
        msg.data = True
        self.conveyor_pub.publish(msg)
        self.get_logger().info("컨베이어 구동 신호 전송 (10초)")
